...
def add_personal_expense(user_id, amount, category, description=""):
    ...
    cursor.execute(
        ...
        VALUES (%s, %s, %s, %s, %s)
        """,
        (user_id, amount, category, description, date.today())
    )
    conn.commit()
    conn.close()
...
def get_total_spend(user_id):
    """
    FIX: Previously fetched ALL rows and summed in Python.
    Now uses a single SQL SUM() — much faster for large datasets.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
